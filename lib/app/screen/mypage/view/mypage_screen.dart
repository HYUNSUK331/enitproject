...
class MyPageView extends GetView<MyPageController> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: Text("My page", style: Theme.of(context).textTheme.headlineSmall),
        /// 나중에 색 죽이기
          ...
          child:  Column(
            children: [
              SizedBox(
                width: 120,
                height: 120,
                child: ClipRRect(borderRadius: BorderRadius.circular(100),child: Image.asset('assets/mypage/proflie.jpg'),),
              ),
              const SizedBox(height: 10),
              Obx(() =>
              Text("${AuthService.to.userModel.value!.name}님 반가워요",style: Theme.of(context).textTheme.headlineSmall),
          ),
              Text("${AuthService.to.userModel.value!.email}",style: Theme.of(context).textTheme.bodyMedium),
              const SizedBox(height: 20),
              SizedBox(
                width: 100,
                child: ElevatedButton(
                  onPressed: (){
                    Get.to(() => const MyPageEditUpdateView(), binding: MyPageEditUpdateBinding());
                  },
                        style: ElevatedButton.styleFrom(
                    backgroundColor: GREEN_BRIGHT_COLOR, side: BorderSide.none, shape: const StadiumBorder()),
              ...
              ProfileMenu(title: "information", icon: LineAwesomeIcons.info, onPress: (){
                Get.dialog(
                  AlertDialog(
                    title: const Column(
                      children: <Widget>[
                        Text("information"), // 타이틀
                      ],
                    ),
                    content: const Column(
                      mainAxisSize: MainAxisSize.min,
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: (){
                    Get.to(() => const AllPreviewScreen(), binding: PreviewBinding());
                  },
                  style: ElevatedButton.styleFrom(
                      backgroundColor: GREEN_BRIGHT_COLOR, side: BorderSide.none, shape: const StadiumBorder()),
                  child: const Text("모든 이야기확인", style: TextStyle(color: Colors.black),
                  ),
                ),
              ),
              const Divider(),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: (){
                    Get.to(() => const MyPageEditDeleteView(), binding: MyPageEditDeleteBinding());
                  },
                  style: ElevatedButton.styleFrom(
                      backgroundColor: Colors.red, side: BorderSide.none, shape: const StadiumBorder()),
                  child: const Text("탈퇴", style: TextStyle(color: Colors.black),
                  ),
                ),
              ),
            ],
...
class ProfileMenu extends StatelessWidget {  ///이런식으로 활용해도 stateless를 사용 하면 안되나?
  const ProfileMenu({
    Key? key,
    required this.title,
    required this.icon,
